...
import { createSetupSession } from "./createSetupSession";

type DisplayModeType = "CUSTOMER_PORTAL" | "EMBEDDED" | "CUSTOM";
...
export default function SetupSessionsDemo() {
  const [customerName, setCustomerName] = useState("");
  const [showEmbeddedSetup, setShowEmbeddedSetup] = useState(false);
  const [customerId, setCustomerId] = useState("");
  const [customerIdEdited, setCustomerIdEdited] = useState(false);
  const [displayMode, setDisplayMode] = useState<DisplayModeType>("CUSTOMER_PORTAL");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [setupStatus, setSetupStatus] = useState<"COMPLETE" | "CANCELED" | null>(null);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!result.success || !result.url) {
        setError(result.error || "Error al crear la sesión de configuración");
        setLoading(false);
        return;
      }
  ...
  const demoFinished = setupStatus === "COMPLETE";
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-8 pb-20 gap-10 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <header className="w-full max-w-md">
        <h1 className="text-2xl font-bold mb-2">
          Sesiones de inscripción con @quentli/js
        </h1>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Este ejemplo muestra cómo usar el SDK oficial de Quentli para
          guardar métodos de pago sin realizar un cobro.
        </p>
      </header>
      ...
      <main className="w-full max-w-md">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          >
            {loading ? (
              "Creando sesión de configuración..."
            ) : (
              <>
          ...
          {setupStatus === "CANCELED" && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 p-3 rounded text-sm mt-3 border border-yellow-300 dark:border-yellow-700">
              La sesión de configuración ha sido cancelada por el usuario.
            </div>
          )}
        </form>
